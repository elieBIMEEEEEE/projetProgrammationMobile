...
class CharacterRepository {
  ...
  Future<List<Character>> fetchCharactersDetails(List<Character> characters) async {
    ...
    List<Future<Character>> characterFutures = characters.sublist(0, sublistEndIndex).map((character) async {
      ...
      return await _performRequestForSingleCharacter(url, character);
    }).toList();
  ...
  Future<Character> _performRequestForSingleCharacter(Uri url, Character characterToUpdate) async {
    final response = await http.get(url);
